...
def _predict(
    ...
    Returns:
        Tuple of (predicted labels, no-correction flags, confidence scores)
    """
    if not srcs:
    ...
    pred_labels = []
    no_corrections = []
    confidences = []  # Store confidence scores for each token
    ...
    for i in itr:
        ...
        # Extract probabilities (assuming outputs has logits or probs)
        pred_probs = torch.softmax(outputs.logits, dim=-1) if hasattr(outputs, 'logits') else outputs.probs
        ...
        for i in range(len(outputs.pred_labels)):
            ...
            for j, idx in enumerate(word_ids(i)):
                ...
                if idx != previous_word_idx:
                    ...
                    conf_score = pred_probs[i][j][outputs.pred_label_ids[i][j]].item() if pred_probs is not None else 1.0
...
def predict(
    ...
    Returns:
        Tuple of (corrected sentences, confidence scores)
    """
    if not srcs:
    ...
    final_conf_scores = [0.0] * len(srcs)  # Store average confidence per sentence
    ...
    for itr in tqdm(range(n_iteration), desc="Correction iterations"):
        ...
        for i, (yes, conf) in enumerate(zip(no_corrections, confidences)):
            if yes:  # No corrections needed
                final_edited_sents[original_sent_idx[i]] = ' '.join(to_be_processed[i]).replace('$START ', '')
                # Average confidence score, default to 1.0 if no edits
                final_conf_scores[original_sent_idx[i]] = sum(conf) / len(conf) if conf else 1.0
            else:
        ...
        confidences = current_confidences  # Update confidences for next iteration
    
    # Finalize remaining sentences
    for i in range(len(to_be_processed)):
        final_edited_sents[original_sent_idx[i]] = ' '.join(to_be_processed[i]).replace('$START ', '')
        final_conf_scores[original_sent_idx[i]] = sum(confidences[i]) / len(confidences[i]) if confidences[i] else 1.0
